...
export function getImages(node: any): any[] {
  ...
  if (node.children && Array.isArray(node.children)) {
    for (const child of node.children) {
      const children = { ...child, folders: node?.name || null }
      images = images.concat(getImages(children));
    }
  }
  return images;
}
...
export function filterFolders(node: TreeNode): TreeNode | null {
  ...
  const filteredChildren = node.children?.map(filterFolders).filter((child): child is TreeNode => child !== null) || [];

  return { ...node, children: filteredChildren };
}

export function getFoldersOnly(tree: TreeNode): TreeNode[] {
  return tree.children?.map(filterFolders).filter((child): child is TreeNode => child !== null) || [];
}
